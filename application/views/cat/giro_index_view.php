    ...
    <div class="card hoverable">
        ...
        <ul class="collection with-header">
            <li class="collection-header">
                <h4 class="center-align">Giros</h4>
            </li>

            <?php foreach ($registros as $reg): ?>
                <li class="collection-item">
                    <div>
                        <input class="check-status" type="checkbox" id="test<?= $reg->idgiro; ?>" data-value='<?= json_encode($reg); ?>' <?= $reg->activo > 0? 'checked': ''; ?>/>
                        <label for="test<?= $reg->idgiro; ?>"><?= $reg->descripcion; ?>&nbsp;(<?= $reg->desctipogiro; ?>)</label>
...
<div id="modal2" class="modal modal-fixed-footer">
    <div class="modal-content">
        ...
        <div class="row">
            <form action="<?= base_url('cat/giro/create'); ?>" method="POST" id="form_create">
                <div class="input-field col s12">
                    <select name="tipogiro_id">
                        <?php foreach ($tipogiros as $tipo): ?>
                            <option value="<?= $tipo->idtipogiro; ?>"><?= $tipo->descripcion; ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <!-- campos ocultos -->
                <input type="hidden" name="idgiro" value="0">
...
<div id="modal1" class="modal modal-fixed-footer">
    <div class="modal-content">
        ...
        <div class="row">
            <form action="<?= base_url('cat/giro/edit'); ?>" method="POST" id="form_edit">
                <div class="input-field col s12">
                    <select id="tipogiro_id" name="tipogiro_id">
                        <?php foreach ($tipogiros as $tipo): ?>
                            <option value="<?= $tipo->idtipogiro; ?>"><?= $tipo->descripcion; ?></option>
                        <?php endforeach; ?>
                    </select>
...
$(document).ready(function() {
    ...
    $(".btn-edit").click(function(event) {
        ...
        //console.log(json);
        $("#idgiro").val(json.idgiro);
        $("#activo").val(json.activo);
        $("#tipogiro_id").val(json.tipogiro_id);
